...
print("=== BUILDING 100% DENTISTI DATASET FROM EXCEL ===")
...
for sheet_name in wb.sheet_names():
    ...
    for r in range(ws.nrows):
        ...
        # Parse Phone / Email / Address metadata rows
        row_str = " ".join(row)
...
print(f"Total Unique Dentisti Master extracted from Excel: {len(doctors_master)}")
...
print(f"Saved {len(final_dentisti_list)} complete doctors into src/data/dentisti_full_excel_master.json")

ambrosino_check = [d for d in final_dentisti_list if 'AMBROSINO' in d['name'].upper()]
print(f"\nAmbrosino records found in master dataset: {len(ambrosino_check)}")
for a in ambrosino_check:
