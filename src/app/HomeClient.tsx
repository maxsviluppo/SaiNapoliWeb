...
export default function HomeClient() {
  ...
  useEffect(() => {
    fetch('/api/admin/save-locales')
      ...
      .catch(err => console.error('Error loading home company data:', err));
  }, []);
            ...
            <div className="space-y-6">
              <div className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <a href={`tel:${(azienda?.phone || "081 06 08 910").replace(/\s+/g, "")}`} className="text-sm text-slate-600 hover:text-sai-blue block mt-0.5">Fisso: {azienda?.phone || "081 06 08 910"}</a>
                  <a href={`tel:${(azienda?.phone_mobile || "+39 393 88 79 849").replace(/\s+/g, "")}`} className="text-sm text-slate-600 hover:text-sai-blue block mt-0.5">Cell: {azienda?.phone_mobile || "+39 393 88 79 849"}</a>
                  <span className="text-[10px] text-slate-400 block mt-0.5">Orari: {azienda?.hours || "Lun-Ven: 09:00 - 18:00"}</span>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <a href={`mailto:${azienda?.email || "saluteambienteigiene@gmail.com"}`} className="text-sm text-slate-600 hover:text-sai-green block mt-0.5">{azienda?.email || "saluteambienteigiene@gmail.com"}</a>
                  {azienda?.pec && (
                    <a href={`mailto:${azienda.pec}`} className="text-[11px] text-slate-500 hover:text-sai-green block mt-0.5">PEC: {azienda.pec}</a>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <span className="text-sm text-slate-600 block mt-0.5">{azienda?.address || "Via Luigi La Vista, 5 - Napoli (Italy)"}</span>
          ...
          <div className="lg:col-span-7">
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-100">
              ...
              <form className="space-y-4" onSubmit={(e) => { 
                e.preventDefault(); 
                alert(`Richiesta inviata con successo! Un tecnico ${azienda?.name || "S.A.I. s.r.l."} la ricontatterà a breve tramite email (${azienda?.email || "saluteambienteigiene@gmail.com"}) o telefono.`); 
              }}>
                ...
                <div className="space-y-1">
                  ...
                  <textarea 
                    rows={4}
                    placeholder="Descrivi brevemente la tua attività e le tue esigenze. Se hai selezionato «Altro», indica qui il servizio richiesto."
