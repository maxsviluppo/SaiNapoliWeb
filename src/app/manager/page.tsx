...
import { 
  ...
  TrendingUp, Activity, AlertTriangle, FileX, FileCheck, Building2, ChevronDown, ChevronUp
} from 'lucide-react';
...
import { generateInitialAmministratori, AmministratoreContract, Condominium, CondominiumDueFrequency, createEmptyCondominium, getCondominiumDisplayName, computeCondominiumDueDate, getCondominiumFrequencyLabel } from '../../data/amministratoriSeed';
...
export default function ManagerDashboard() {
  ...
  const [adminCondominiumSearch, setAdminCondominiumSearch] = useState('');
  const [expandedAdminCondominiumId, setExpandedAdminCondominiumId] = useState<string | null>(null);
  const [adminCondominiumsPanelOpen, setAdminCondominiumsPanelOpen] = useState(false);
  ...
  useEffect(() => {
    setAdminCondominiumsPanelOpen(false);
    setExpandedAdminCondominiumId(null);
    setAdminCondominiumSearch('');
  }, [selectedAmministratore?.id]);
  ...
  const updateSelectedCondominium = (condId: string, patch: Partial<Condominium>) => {
    if (!selectedAmministratore) return;
    setSelectedAmministratore({
      ...selectedAmministratore,
      condominiums: selectedAmministratore.condominiums.map(c => {
        if (c.id !== condId) return c;
        const merged = { ...c, ...patch };
        const manualDueEdit = patch.dueDate !== undefined && !('invoiceDate' in patch) && !('dueFrequency' in patch) && !('customFrequencyDays' in patch);
        const shouldRecalcDue = !manualDueEdit && (
          patch.invoiceDate !== undefined ||
          patch.dueFrequency !== undefined ||
          patch.customFrequencyDays !== undefined
        );
        if (shouldRecalcDue && merged.invoiceDate) {
          if (merged.dueFrequency === 'personalizzato' && !merged.customFrequencyDays) {
            return merged;
          }
          merged.dueDate = computeCondominiumDueDate(
            merged.invoiceDate,
            merged.dueFrequency || 'annuale',
            merged.customFrequencyDays
          );
        }
        return merged;
      }),
  ...
  const addCondominiumToSelected = () => {
    if (!selectedAmministratore) return;
    const newCond = createEmptyCondominium('Nuovo Condominio');
    setSelectedAmministratore({
      ...selectedAmministratore,
      condominiums: [newCond, ...selectedAmministratore.condominiums],
    });
    setAdminCondominiumsPanelOpen(true);
    setExpandedAdminCondominiumId(newCond.id);
    setAdminCondominiumSearch('');
  };

  const removeCondominiumFromSelected = (condId: string) => {
    if (!selectedAmministratore) return;
    setSelectedAmministratore({
      ...selectedAmministratore,
      condominiums: selectedAmministratore.condominiums.filter(c => c.id !== condId),
    });
    if (expandedAdminCondominiumId === condId) setExpandedAdminCondominiumId(null);
  };

  const filteredSelectedCondominiums = useMemo(() => {
    if (!selectedAmministratore) return [];
    const q = adminCondominiumSearch.trim().toLowerCase();
    if (!q) return selectedAmministratore.condominiums;
    return selectedAmministratore.condominiums.filter(c => {
      const blob = [
        c.name, c.address, c.city, c.partitaIva, c.codiceFiscale,
        c.invoiceNumber, c.invoiceDate, c.dueDate, c.dueFrequency,
        c.customFrequencyDays?.toString(), c.portiere, c.portierePhone, c.notes,
      ].join(' ').toLowerCase();
      return blob.includes(q);
    });
  }, [selectedAmministratore, adminCondominiumSearch]);
                    ...
                    <button 
                      onClick={() => {
                        ...
                        const newAmm: AmministratoreContract = {
                          ...
                          startDate: today,
                          condominiums: [createEmptyCondominium('Condominio Parco Sole')],
                        ...
                        return (
                          <tr key={c.id} className={`transition-colors ${isOverdueActive ? 'bg-red-50/90 hover:bg-red-100/90 text-red-900 border-l-4 border-red-500 font-bold' : 'hover:bg-purple-50/30'}`}>
                            ...
                            <td className="px-1 py-2 text-center align-top">
                              <span className="inline-block px-1.5 py-0.5 bg-emerald-50 text-emerald-700 rounded text-[9px] font-black" title={c.condominiums.slice(0, 3).map(getCondominiumDisplayName).join(', ')}>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] max-w-[96vw] xl:max-w-[1450px] w-full p-6 md:p-8 shadow-2xl border border-slate-100 flex flex-col text-left max-h-[94vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-6">
              ...
              <button 
                onClick={() => { setAdminCondominiumSearch(''); setExpandedAdminCondominiumId(null); setSelectedAmministratore(null); }}
            ...
            >
              {/* CONDOMINI GESTITI — fisarmonica chiusa di default */}
              <div className="border border-emerald-200 rounded-2xl overflow-hidden bg-white shadow-sm">
                <button
                  type="button"
                  onClick={() => setAdminCondominiumsPanelOpen(prev => !prev)}
                  className={`w-full px-4 py-3 bg-gradient-to-r from-emerald-50 to-teal-50 flex items-center justify-between gap-3 text-left transition-colors hover:from-emerald-100/80 hover:to-teal-100/80 ${adminCondominiumsPanelOpen ? 'border-b border-emerald-100' : ''}`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h4 className="font-black text-emerald-900 uppercase text-[10px] tracking-wider">
                          Condomini Gestiti ({selectedAmministratore.condominiums.length})
                        </h4>
                        {selectedAmministratore.condominiums.some(c => c.dueDate && getExpiryStatus(c.dueDate) === 'scaduto') && (
                          <span className="px-1.5 py-0.5 bg-red-600 text-white rounded text-[8px] font-black uppercase">
                            {selectedAmministratore.condominiums.filter(c => c.dueDate && getExpiryStatus(c.dueDate) === 'scaduto').length} scaduti
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] text-emerald-700/80 font-medium truncate">
                        {adminCondominiumsPanelOpen ? 'Frequenza scadenze, fatture, portieri' : 'Clicca la freccia per aprire l\'elenco condomini'}
                      </p>
                    </div>
                  </div>
                  <ChevronDown className={`w-5 h-5 text-emerald-600 shrink-0 transition-transform duration-200 ${adminCondominiumsPanelOpen ? 'rotate-180' : ''}`} />
                </button>

                {adminCondominiumsPanelOpen && (
                  <>
                    <div className="px-4 py-2.5 bg-emerald-50/50 border-b border-emerald-100 flex items-center justify-end gap-2 flex-wrap">
                      <div className="relative">
                        <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-emerald-400" />
                        <input
                          type="text"
                          value={adminCondominiumSearch}
                          onChange={(e) => setAdminCondominiumSearch(e.target.value)}
                          placeholder="Cerca condominio..."
                          className="pl-8 pr-3 py-2 bg-white border border-emerald-200 rounded-xl text-[11px] font-medium w-44 sm:w-52 focus:outline-none focus:border-emerald-500"
                        />
                      </div>
                      <button
                        type="button"
                        onClick={addCondominiumToSelected}
                        className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-[10px] flex items-center gap-1.5 shadow-sm"
                      >
                        <Plus className="w-3.5 h-3.5" /> Aggiungi
                      </button>
                    </div>

                {filteredSelectedCondominiums.length === 0 ? (
                  <div className="px-6 py-10 text-center text-slate-400 text-xs font-medium">
                    {adminCondominiumSearch ? 'Nessun condominio corrisponde alla ricerca.' : 'Nessun condominio censito. Usa "Aggiungi" per inserirne uno.'}
                  </div>
                ) : (
                  <div className="max-h-[420px] overflow-y-auto divide-y divide-emerald-50">
                    {filteredSelectedCondominiums.map((cond, index) => {
                      const isOpen = expandedAdminCondominiumId === cond.id;
                      const dueStatus = cond.dueDate ? getExpiryStatus(cond.dueDate) : null;
                      const isOverdue = dueStatus === 'scaduto';
                      const isSoon = dueStatus === 'in_scadenza';
                      const freq = cond.dueFrequency || 'annuale';
                      return (
                        <div
                          key={cond.id}
                          className={`transition-colors border-l-4 ${
                            isOverdue
                              ? 'bg-red-50 border-l-red-500 hover:bg-red-100/80'
                              : isSoon
                                ? 'bg-amber-50/70 border-l-amber-400 hover:bg-amber-50'
                                : isOpen
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => setExpandedAdminCondominiumId(isOpen ? null : cond.id)}
                            className="w-full px-4 py-3 flex items-start gap-3 text-left"
                          >
                            <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0 mt-0.5 ${
                              isOverdue ? 'bg-red-200 text-red-800' : isSoon ? 'bg-amber-200 text-amber-900' : 'bg-emerald-100 text-emerald-700'
                            }`}>
                              {index + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                <span className={`font-bold text-xs truncate max-w-full ${isOverdue ? 'text-red-900' : 'text-slate-900'}`}>
                                  {cond.name || 'Condominio senza nome'}
                                </span>
                                <span className="px-1.5 py-0.5 bg-white/80 border border-slate-200 rounded text-[8px] font-black uppercase text-slate-600">
                                  {getCondominiumFrequencyLabel(freq, cond.customFrequencyDays)}
                                </span>
                                {isSoon && (
                                  <span className="px-1.5 py-0.5 bg-amber-500 text-white rounded text-[8px] font-black uppercase">In scadenza</span>
                                )}
                                {isOverdue && (
                                  <span className="px-1.5 py-0.5 bg-red-600 text-white rounded text-[8px] font-black uppercase animate-pulse">Scaduto</span>
                                )}
                              </div>
                              <div className={`flex flex-wrap gap-x-3 gap-y-0.5 mt-1 text-[10px] font-medium ${isOverdue ? 'text-red-700' : 'text-slate-500'}`}>
                                {cond.address && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{cond.address}{cond.city ? `, ${cond.city}` : ''}</span>}
                                {cond.partitaIva && <span>P.IVA {cond.partitaIva}</span>}
                                {cond.codiceFiscale && <span>CF {cond.codiceFiscale}</span>}
                                {cond.portiere && <span>Portiere: {cond.portiere}</span>}
                                {cond.invoiceNumber && <span>Fatt. {cond.invoiceNumber}{cond.invoiceDate ? ` del ${cond.invoiceDate}` : ''}</span>}
                                {cond.dueDate && <span className={isOverdue ? 'font-black' : ''}>Scad. {cond.dueDate}</span>}
                              </div>
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                              <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); removeCondominiumFromSelected(cond.id); }}
                                className="p-1.5 rounded-lg text-red-400 hover:bg-red-50 hover:text-red-600"
                                title="Elimina condominio"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                              {isOpen ? <ChevronUp className={`w-4 h-4 ${isOverdue ? 'text-red-600' : 'text-emerald-600'}`} /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                            </div>
                          </button>

                          {isOpen && (
                            <div className="px-4 pb-4 pl-[3.25rem] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                              <div className="sm:col-span-2 lg:col-span-3">
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Nome Condominio</label>
                                <input type="text" value={cond.name} onChange={(e) => updateSelectedCondominium(cond.id, { name: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold" />
                              </div>
                              <div className="sm:col-span-2">
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Indirizzo</label>
                                <input type="text" value={cond.address} onChange={(e) => updateSelectedCondominium(cond.id, { address: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Comune / Città</label>
                                <input type="text" value={cond.city} onChange={(e) => updateSelectedCondominium(cond.id, { city: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Partita IVA</label>
                                <input type="text" value={cond.partitaIva || ''} onChange={(e) => updateSelectedCondominium(cond.id, { partitaIva: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Codice Fiscale</label>
                                <input type="text" value={cond.codiceFiscale || ''} onChange={(e) => updateSelectedCondominium(cond.id, { codiceFiscale: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">N° Fattura</label>
                                <input type="text" value={cond.invoiceNumber || ''} onChange={(e) => updateSelectedCondominium(cond.id, { invoiceNumber: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Data Fattura</label>
                                <input type="date" value={cond.invoiceDate || ''} onChange={(e) => updateSelectedCondominium(cond.id, { invoiceDate: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Frequenza Scadenza</label>
                                <select
                                  value={freq}
                                  onChange={(e) => updateSelectedCondominium(cond.id, { dueFrequency: e.target.value as CondominiumDueFrequency })}
                              ...
                              {freq === 'personalizzato' && (
                                <div>
                                  <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Giorni (personalizzato)</label>
                                  <input
                                    type="number"
                                    min={1}
                                    value={cond.customFrequencyDays ?? 30}
                                    onChange={(e) => updateSelectedCondominium(cond.id, { customFrequencyDays: parseInt(e.target.value) || 30 })}
                              ...
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">
                                  Scadenza {freq === 'personalizzato' ? '(manuale o auto da giorni)' : '(calcolata)'}
                                </label>
                                <input
                                  type="date"
                                  value={cond.dueDate || ''}
                                  onChange={(e) => updateSelectedCondominium(cond.id, { dueDate: e.target.value })}
                                  className={`w-full bg-white border rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold ${
                                    isOverdue ? 'border-red-400 text-red-800 bg-red-50' : 'border-slate-200'
                                  }`}
                                />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Portiere di Riferimento</label>
                                <input type="text" value={cond.portiere || ''} onChange={(e) => updateSelectedCondominium(cond.id, { portiere: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold" />
                              </div>
                              <div>
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Tel. Portiere</label>
                                <input type="text" value={cond.portierePhone || ''} onChange={(e) => updateSelectedCondominium(cond.id, { portierePhone: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold" />
                              </div>
                              <div className="sm:col-span-2 lg:col-span-3">
                                <label className="text-[9px] font-black uppercase tracking-wider text-slate-400 block mb-1">Note Condominio</label>
                                <textarea rows={2} value={cond.notes || ''} onChange={(e) => updateSelectedCondominium(cond.id, { notes: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-medium resize-none" />
                              </div>
                            </div>
                          )}
                        </div>
